import random as r
import string
xing = ['陳','林','黃','張','李','王','吳','劉','蔡','楊','許','鄭','謝','郭',
        '洪','邱','曾','廖','賴','徐','周','葉','蘇','莊','江','呂','何','羅',
        '高','蕭','潘','朱','簡','鍾','彭','游','詹','胡','施','沉','余','趙',
        '盧','梁','顏','柯','孫','魏','翁','戴','范','宋','方','鄧','杜','傅',
        '侯','曹','溫','薛','丁','馬','蔣','唐','卓','藍','馮','姚','石','董',
        '紀','歐','程','連','古','汪','湯','姜','田','康','鄒','白','塗','尤',
        '巫','韓','龔','嚴','袁','鍾','黎','金','阮','陸','倪','夏','童','邵',
        '柳','錢']
ming = [
'的', '一', '是', '了', '我', '不', '人', '在', '他', '有', '這', '個', '上', '們', '來', '到', '時', '大', '地', '為',
'子', '中', '你', '說', '生', '國', '年', '著', '就', '那', '和', '要', '她', '出', '也', '得', '裡', '後', '自', '以',
'會', '家', '可', '下', '而', '過', '天', '去', '能', '對', '小', '多', '然', '於', '心', '學', '麼', '之', '都', '好',
'看', '起', '發', '當', '沒', '成', '只', '如', '事', '把', '還', '用', '第', '樣', '道', '想', '作', '種', '開', '美',
'總', '從', '無', '情', '己', '面', '最', '女', '但', '現', '前', '些', '所', '同', '日', '手', '又', '行', '意', '動',
'方', '期', '它', '頭', '經', '長', '兒', '回', '位', '分', '愛', '老', '因', '很', '給', '名', '法', '間', '斯', '知',
'世', '什', '兩', '次', '使', '身', '者', '被', '高', '已', '親', '其', '進', '此', '話', '常', '與', '活', '正', '感',
'見', '明', '問', '力', '理', '爾', '點', '文', '幾', '定', '本', '公', '特', '做', '外', '孩', '相', '西', '果', '走',
...
'勝', '份', '汽', '右', '洋', '範', '床', '舞', '秘', '午', '登', '樓', '貴', '吸', '責', '例', '追', '較', '職', '屬',
'漸', '左', '錄', '絲', '牙', '黨', '繼', '托', '趕', '章', '智', '沖', '葉', '胡', '吉', '賣', '堅', '喝', '肉', '遺',
'救', '修', '松', '臨', '藏', '擔', '戲', '善', '衛', '藥', '悲', '敢', '靠', '伊', '村', '戴', '詞', '森', '耳', '差',
'短', '祖', '雲', '規', '窗', '散', '迷', '油', '舊', '適', '鄉', '架', '恩', '投', '彈', '鐵', '博', '雷', '府', '壓',
'超', '負', '勒', '雜', '醒', '洗', '采', '毫', '嘴', '畢', '九', '冰', '既', '狀', '亂', '景', '席', '珍', '童', '頂',
'派', '素', '脫', '農', '疑', '練', '野', '按', '犯', '拍', '征', '壞', '骨', '餘', '承', '置', '臓', '彩', '燈', '巨',
'琴', '免', '環', '姆', '暗', '換', '技', '翻', '束', '增', '忍', '餐', '洛', '塞', '缺', '憶', '判', '歐', '層', '付',
'陣', '瑪', '批', '島', '項', '狗', '休', '懂', '武', '革', '良', '惡', '戀', '委', '擁', '娜', '妙', '探', '呀', '營',
'退', '搖', '弄', '桌', '熟', '諾', '宣', '銀', '勢', '獎', '宮', '忽', '套', '康', '供', '優', '課', '鳥', '喊', '降',
'夏', '困', '劉', '罪', '亡', '鞋', '健', '模', '敗', '伴', '守', '揮', '鮮', '財', '孤', '槍', '禁', '恐', '夥', '傑',
'跡', '妹', '藸', '遍', '蓋', '副', '坦', '牌', '江', '順', '秋', '薩', '菜', '劃', '授', '歸', '浪', '聽', '凡', '預',
'奶', '雄', '升', '碃', '編', '典', '袋', '萊', '含', '盛', '濟', '蒙', '棋', '端', '腿', '招', '釋', '介', '燒', '誤',
'乾', '坤']
...
#身分證
def B():
	return A()+a+"2"+''.join(r.choice(string.digits) for x in range(7))
...
#名字              
def name():
	return r.choice(xing) + r.choice(ming) + r.choice(ming)

#電話
def phone():
	return "09"+''.join(r.choice(string.digits) for x in range(8))

#年
def gen_year():
    return str(r.randint(1900, 2020))

#月/日
def gen_birthday():
        month = r.randint(1, 12)
        day = r.randint(1, 30)
        return  gen_year()+"/"+str("%02d" % (month))+"/"+str("%02d" % (day))
##def address():

print("姓名: " +name())
print("性別: " +male())
print("生日: "+gen_birthday())
print("身分證: "+B())
print("電話: "+phone())
##print("地址: "+address())
